...
References/ORAN_DHCP_USER_GUIDE.md       # guide v4.0.0, current as of generator v2.2.3
...
**Keep the user guide in sync.** `References/ORAN_DHCP_USER_GUIDE.md` was rewritten for v2.2.3 on 2026-08-24 (guide v4.0.0) and now documents `lease_profiles`, `ca_ra_profiles`, optional per-class ranges, the CA/RA sub-option codes, and `--deploy`/`--restart`. It had previously drifted two minor versions behind, which is how the 2.2.x features went undocumented — when you change behaviour, update the guide, the man page (`pkg/usr/share/man/man1/`, still stamped 2.2.2), and `changelog.gz` in the same change.
